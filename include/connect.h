...
class Conn {
  ...
  public:
    ...
    [[nodiscard]] int get_fd() const { return m_f.data(); }
    [[nodiscard]] short int get_event() const {
        if (m_state == ConnState::STATE_REQ) { return POLLIN; }
        if (m_state == ConnState::STATE_RES) { return POLLOUT; }
        assert(false);
        return 0;
    }
    [[nodiscard]] bool is_end() const { return m_state == ConnState::STATE_END; }
