...
const uint64_t k_idle_timeout_ms = static_cast<const uint64_t>(5 * 1000);
...
class Server {
  ...
  public:
    ...
    int32_t acceptConn() {
        ...
        int connfd = accept(m_f.data(), (struct sockaddr *)&client_addr, &socklen);
        if(connfd < 0) {
            msg("accept() error");
            return -1;
        }
    ...
    uint32_t next_timer_ms() {
        ...
        if(!head.empty()) {
            ...
            next_us = std::min(next_us, next->idle_start + k_idle_timeout_ms * 1000);
        }
    ...
    void join() {
        // fd是套接字
        // set the listen fd to non_blocking mode
        m_f.set_nb();
    ...
    void process_timers() {
        ...
        while(!head.empty()) {
            ...
            while(!heap.empty() && heap.get_min() < now_us) {
                core::Entry *ent = container_of(heap.get_min_ref(), core::Entry, heap_idx);
                HNode *node = core::m_map.pop(&ent->node, [](HNode *lhs, HNode *rhs) {
                    return lhs == rhs;
                });
