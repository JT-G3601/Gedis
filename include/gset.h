...
class ZNodeCollection {
  ...
  public:
    ...
    [[nodiscard]] iterator cbegin() const { return {data_, 0}; }
    [[nodiscard]] iterator cend() const { return {data_, length_}; }

    [[nodiscard]] int64_t size() const { return length_; }
...
class ZSet {
  private:
    ...
    void tree_add(ZNode *node) {
        ...
        while(true) {
            avl::AVLNode **from = zless(&node->avl_node, cur) ? &cur->left
                                                              : &cur->right;
            if(!*from) {
                *from = &node->avl_node;
  ...
  public:
    ...
    ZNodeCollection query(double score, const std::string &name, int64_t offset, int64_t limit) {
        avl::AVLNode *found = nullptr;
        avl::AVLNode *cur = avl_root;
        while (cur) {
            if (zless(cur, score, name)) {
