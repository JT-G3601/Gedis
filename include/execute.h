...
void out_arr(Bytes &out, uint32_t n) {
    out.appendNumber(static_cast<uint32_t>(SerType::SER_ARR), 1);
...
namespace core {
    ...
    struct Entry {
        ...
        void set_ttl(int64_t ttl_ms) {
            if(ttl_ms < 0 && heap_idx != 0) {
                ...
                size_t const pos = heap_idx;
                m_heap.del(pos);
                heap_idx = 0;
            }
            else if(ttl_ms >= 0) {
                size_t const pos = heap_idx;
                uint64_t const now_time = get_monotonic_usec() + (uint64_t)ttl_ms * 1000;
                if(pos == 0) { m_heap.push(now_time, &heap_idx); }
                else { m_heap.set(pos, now_time); }
            }
        }
    };
    ...
    void set(const std::string &k, const std::string &v) {
        ...
        if(node) {
            ...
            auto *ent = new Entry{std::move(key.key), v, key.node.hcode};
