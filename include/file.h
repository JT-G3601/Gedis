...
class File {
  ...
  public:
    ...
    [[nodiscard]] int data() const { return m_fd; }
    void set_nb() const {
        fd_set_nb(m_fd);
    }
    ...
    bool readByte_b(Bytes &bytes, size_t count) const {
        ...
        // 注意这里已经对Bytes进行了resize(不然没空间装), 在err后会还原size
