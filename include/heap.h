...
struct HeapItem {
    ...
    size_t *ref{nullptr};
    HeapItem() = default;
    HeapItem(uint64_t _val, size_t *_ref) : val(_val), ref(_ref) {}
...
class Heap {
  ...
  public:
    ...
    [[nodiscard]] uint64_t get_min() const {
        assert(!empty());
        return data[1].val;
    }
    [[nodiscard]] uint64_t get(size_t pos) const {
        assert(!empty());
        assert(1 <= pos && pos < data.size());
        return data[pos].val;
    }
    [[nodiscard]] size_t *get_min_ref() const {
        assert(!empty());
        return data[1].ref;
    }
    void del(size_t pos) {
    ...
    void push(uint64_t val, size_t *entry_heap_idx) {
        data.emplace_back(val, entry_heap_idx);
        update(data.size() - 1);
    }
